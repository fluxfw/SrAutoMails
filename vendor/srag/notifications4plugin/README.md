...
<#x>
<?php
\srag\Notifications4Plugin\SrAutoMails\x\Repository::getInstance()->installTables();
?>
```
...
```php
if (\srag\Notifications4Plugin\SrAutoMails\x\Notification\Repository::getInstance()->migrateFromOldGlobalPlugin(x::TEMPLATE_NAME) === null) {

	$notification = \srag\Notifications4Plugin\SrAutoMails\x\Notification\Repository::getInstance()->factory()->newInstance();

	$notification->setName(x::TEMPLATE_NAME);

	// TODO: Fill $notification with your default values

	\srag\Notifications4Plugin\SrAutoMails\x\Notification\Repository::getInstance()->storeNotification($notification);
}
```
